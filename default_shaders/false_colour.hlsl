...
//
// This is an instrument, not an effect, so most of the shader quality pass is
// deliberately not applied here:
//   - The band edges stay hard. A monitoring tool has to answer "which side of
//     the line is this pixel on" with no ambiguity, and an anti-aliased edge
//     turns the answer into a gradient you cannot read off the screen.
//   - No tonemap and no dither. Both would move code values, and the code value
//     is the measurement.
//   - The band test runs on the gamma-encoded signal, because the thresholds are
//     IRE levels off the signal as recorded, not scene radiance.
...
float4 main(PS_INPUT input) : SV_TARGET {
    ...
    // A stop is a doubling of light, so the offset has to be applied in linear
    // light and the result re-encoded. Scaling the code values directly (the
    // previous behaviour) meant +1 stop moved a midtone by 2x its encoded value,
    // which is nearer three stops, and the readout could not be trusted.
    float3 exposed = col.rgb;
    if (abs(Exposure) > 1e-4) {
        exposed = spLinearToSrgb(spSrgbToLinear(col.rgb) * exp2(Exposure));
    }
